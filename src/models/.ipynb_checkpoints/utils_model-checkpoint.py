...
class BertAttention(nn.Module):
    ...
    def forward(self, hidden_states, context, attention_mask=None):
        ...
        global  _glob_counter
        #This section is only for visuallizing attention
        for i in range(attention_probs.shape[1]):
            #print(attention_probs[0,i,:20,:20])
            m = attention_probs[0,i]
            
            #a = np.random.random((16, 16))
            #m = torch.cat((m[:50,:30],m[:50,77*2:77*2+24]),dim = 1)
            m = torch.cat((m[:30,:50],m[77*2:77*2+24,:50]),dim = 0)
            
            sns.heatmap(m.detach().cpu().numpy(),cmap = "magma", cbar_kws = dict(use_gridspec=False,location="bottom"))
            start,end = plt.gca().get_xlim()
            #print(start,end)
            #s()
            plt.xticks([start,end])
            plt.gca().set_xticklabels([str(int(start)), str(int(end))]) 
            start,end = plt.gca().get_ylim()
            plt.yticks([start,end])
            plt.gca().set_yticklabels([str(int(start)), str(int(end))]) 
            #plt.imshow(, cmap='hot', interpolation='nearest',vmax=1)
            if attention_probs.shape[-1] != attention_probs.shape[-2] and attention_probs.shape[-1] == 128 :
                if i == 0:
                    print("x",sum(attention_mask[0][0][0]==0))
                print("vals",list(m[0][:])) 
                plt.ylabel("CLIP-T")
                plt.xlabel("BERT")
                plt.savefig(f"./analysis/novlm/xattn_head{str(i)}_layer{str(int(_glob_counter))}.pdf",bbox_inches='tight')
            elif attention_probs.shape[-1] == 128 and attention_probs.shape[-2] == 128:
                if i == 0:
                    print("txt",sum(attention_mask[0][0][0]==0))
                plt.savefig(f"./analysis/novlm/txtattn_head{str(i)}_layer{str(int(_glob_counter))}.pdf",mbbox_inches='tight')
            elif attention_probs.shape[-1] != 128 and attention_probs.shape[-2] != 128:
                if i == 0:
                    print("vis",sum(attention_mask[0][0][0]==0))
                plt.savefig(f"./analysis/novlm/visattn_head{str(i)}_layer{str(int(_glob_counter))}.pdf",bbox_inches='tight')
            plt.clf()
        _glob_counter += 0.25
...
class BertCrossattLayer(nn.Module):
    def __init__(self, config1, config2):
        super().__init__()
        self.att = BertAttention(config1, ctx_dim = config2.hidden_size)
        self.output = BertAttOutput(config1)
        

    def forward(self, input_tensor, ctx_tensor, ctx_att_mask=None):
        #print(input_tensor.shape,ctx_tensor.shape)
...
class LXRTXLayer(nn.Module):
    def __init__(self, config,visconfig):
        ...
        self.visual_attention = BertCrossattLayer(visconfig,config)
        self.language_attention = BertCrossattLayer(config,visconfig)

        # Self-attention Layers
        self.lang_self_att = BertSelfattLayer(config)
        self.visn_self_att = BertSelfattLayer(visconfig)

        # Intermediate and Output Layers (FFNs)
        self.lang_inter = BertIntermediate(config)
        self.lang_output = BertOutput(config)
        self.visn_inter = BertIntermediate(visconfig)
        self.visn_output = BertOutput(visconfig)

    def cross_att(self, lang_input, lang_attention_mask, visn_input, visn_attention_mask):
        # Cross Attention
        lang_att_output = self.language_attention(lang_input, visn_input, ctx_att_mask=visn_attention_mask)
...
class LXRTEncoder(nn.Module):
    def __init__(self, config):
        ...
        print("LXRT encoder with %d l_layers, %d x_layers." %
              (self.num_l_layers, self.num_x_layers))
        ...
        self.vis_model, _ = load_clip('ViT-B/32', "cuda")
        ...
        self.x_layers = nn.ModuleList(
            [LXRTXLayer(config,vis_config) for _ in range(self.num_x_layers)]
        )
    def forward(self, lang_feats, lang_attention_mask, vis_input_ids,
        ...
        # Run language layers
        #print(lang_feats.shape,lang_attention_mask.shape)
        
        for layer_module in self.layer:
            lang_feats = layer_module(lang_feats, lang_attention_mask)

        # Run relational layers
        visn_feats = self.vis_model.encode_text(vis_input_ids)
        #visn_feats=visn_feats.unsqueeze(1).float() # sequence of 1
        #print(lang_feats.shape,visn_feats.shape)
...
class BertLMPredictionHead(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        #print(hidden_states.shape,self.decoder,self.bias.shape)
        #print(self.decoder(hidden_states).shape,self.bias)
...
class BertPreTrainedModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, state_dict=None, cache_dir=None,
        ...
        try:
            resolved_archive_file = cached_path(archive_file, cache_dir="./") #cache_dir = cache_dir
...
class LXRTModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None,vis_input_ids=None, attention_mask=None,
